    ...
    <div>
      ...
      <form className="w-full max-w-sm bg-gray-900 text-white backdrop-blur-sm bg-opacity-70 py-12 px-8 rounded-lg shadow-lg">
        <div className="mb-6 text-left space-y-1">
          <h2 className="text-xl font-semibold">Welcome, <label className="font-normal">Please Sign In!</label></h2>
        ...
        <input
          ...
          className="w-full py-2 mb-6 border-b border-white bg-transparent bg-opacity-20 focus:outline-none focus:border-purple-500 transition duration-300"
        />
        <button onClick={handleSubmit} className="w-full bg-purple-800 rounded-md hover:bg-purple-900 text-white p-2 font-semibold transition duration-300">
          Sign In
        </button>
        <p className="text-center text-sm text-gray-500 mt-4">
          Don't have an account? <button onClick={handleSignUp} className="text-purple-500">Sign up</button>
        </p>
      </form>
    </div>
  );
